...
export default function Implantation({
    ...
    // --- NEW: EXCLUSIVE PLANTATION MODE STATE ---
    const [activePlantation, setActivePlantation] = useState<'zigzag' | 'snake' | 'grid' | null>(null);
    ...
    // --- NEW: EXCLUSIVE PLANTATION TOGGLE HANDLER ---
    const togglePlantation = (type: 'zigzag' | 'snake' | 'grid') => {
        if (activePlantation === type) {
            // Already active → Turn OFF (unlock)
            setActivePlantation(null);
            setLayoutType('free'); // Return to free mode
        } else {
            // Activate THIS plantation (lock)
            setActivePlantation(type);
            setLayoutType(type);
        }
    };
                                ...
                                <>
                                    ...
                                    {isLayoutMenuOpen && (
                                        <div className="absolute top-full left-0 mt-2 p-1.5 bg-white rounded-xl shadow-xl border border-slate-200 z-50 flex items-center gap-2 animate-in slide-in-from-top-2 min-w-[300px]">
                                            {/* Standard Layouts - WITH EXCLUSIVE MODE */}
                                            <div className="flex items-center gap-1 pr-2 border-r border-slate-100">
                                                {/* U (Standard) Plantation */}
                                                <button
                                                    onClick={() => togglePlantation('zigzag')}
                                                    disabled={activePlantation !== null && activePlantation !== 'zigzag'}
                                                    className={`relative flex items-center gap-1 px-2 py-1.5 rounded-md transition-all text-[9px] font-bold ${activePlantation === 'zigzag'
                                                            ? 'bg-indigo-600 text-white shadow-md ring-2 ring-indigo-200'
                                                            : layoutType === 'zigzag' && activePlantation === null
                                                                ? 'bg-indigo-50 text-indigo-600'
                                                                : activePlantation !== null
                                                                    ? 'text-slate-300 cursor-not-allowed opacity-40'
                                                                    : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'
                                                        }`}
                                                    title={activePlantation === 'zigzag' ? "Déverouiller U (Cliquer pour désactiver)" : activePlantation !== null ? "Verrouillé - Désactiver l'autre plantation d'abord" : "U (Standard)"}
                                                >
                                                    {activePlantation === 'zigzag' && <Magnet className="w-3 h-3" />}
                                                    <span>U (Standard)</span>
                                                </button>

                                                {/* Serpent Plantation */}
                                                <button
                                                    onClick={() => togglePlantation('snake')}
                                                    disabled={activePlantation !== null && activePlantation !== 'snake'}
                                                    className={`relative flex items-center gap-1 px-2 py-1.5 rounded-md transition-all text-[9px] font-bold ${activePlantation === 'snake'
                                                            ? 'bg-indigo-600 text-white shadow-md ring-2 ring-indigo-200'
                                                            : layoutType === 'snake' && activePlantation === null
                                                                ? 'bg-indigo-50 text-indigo-600'
                                                                : activePlantation !== null
                                                                    ? 'text-slate-300 cursor-not-allowed opacity-40'
                                                                    : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'
                                                        }`}
                                                    title={activePlantation === 'snake' ? "Déverouiller Serpent (Cliquer pour désactiver)" : activePlantation !== null ? "Verrouillé - Désactiver l'autre plantation d'abord" : "Serpent"}
                                                >
                                                    {activePlantation === 'snake' && <Magnet className="w-3 h-3" />}
                                                    <span>Serpent</span>
                                                </button>

                                                {/* Grid Plantation */}
                                                <button
                                                    onClick={() => togglePlantation('grid')}
                                                    disabled={activePlantation !== null && activePlantation !== 'grid'}
                                                    className={`relative flex items-center gap-1 p-1.5 rounded-md transition-all ${activePlantation === 'grid'
                                                            ? 'bg-indigo-600 text-white shadow-md ring-2 ring-indigo-200'
                                                            : layoutType === 'grid' && activePlantation === null
                                                                ? 'bg-indigo-50 text-indigo-600'
                                                                : activePlantation !== null
                                                                    ? 'text-slate-300 cursor-not-allowed opacity-40'
                                                                    : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'
                                                        }`}
                                                    title={activePlantation === 'grid' ? "Déverouiller Grille (Cliquer pour désactiver)" : activePlantation !== null ? "Verrouillé - Désactiver l'autre plantation d'abord" : "Grille"}
                                                >
                                                    {activePlantation === 'grid' && <Magnet className="w-3 h-3" />}
                                                    <GridIcon className="w-4 h-4" />
                                                </button>
                                            </div>


                                            {/* Free Mode Assistants */}
                                            ...
                                            <button
                                                onClick={() => { setLayoutType('free'); setIsManualMode(true); }}
